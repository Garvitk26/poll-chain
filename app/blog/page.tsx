  ...
  return (
    <main className="min-h-[calc(100vh-64px)] py-20 px-4 md:px-8 bg-[#00080f]">
      <div className="max-w-4xl mx-auto space-y-12">
        <div className="space-y-4 border-b border-rose-500/10 pb-8">
        ...
        <div className="space-y-6">
          {posts.map(post => (
             <div key={post.id} className="bg-[#000d1a] border border-rose-500/10 rounded-xl p-6 md:p-8 hover:border-rose-500/30 transition-colors group cursor-pointer">
               <div className="flex items-center gap-3 mb-4">
                 <span className="text-xs font-semibold text-rose-400 tracking-wider uppercase">{post.category}</span>
                 <span className="text-xs text-slate-500">{post.date}</span>
               </div>
               <h2 className="text-2xl font-bold text-slate-200 mb-4 group-hover:text-rose-300 transition-colors">{post.title}</h2>
               <div className="flex items-center text-violet-400 text-sm font-medium gap-2">
