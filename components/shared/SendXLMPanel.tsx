...
export default function SendXLMPanel({ defaultMemo = '', onSuccess, compact = false }: SendXLMPanelProps) {
        ...
        <CardContent className="flex flex-col items-center justify-center p-12 space-y-6">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-lg font-black uppercase tracking-tight italic">
              {step === 'BUILDING' && 'Encoding Transaction...'}
              {step === 'SIGNING' && 'Awaiting Consensus...'}
              {step === 'BROADCASTING' && 'Ledger Integration...'}
            </h3>
            <p className="text-xs text-muted-foreground/60 font-bold uppercase tracking-widest leading-relaxed">
              ...
              {step === 'BROADCASTING' && 'Propagating to Stellar Testnet'}
            </p>
          </div>
          {step === 'SIGNING' && (
  ...
  if (step === 'SUCCESS' && txResult?.success) {
    return (
      <Card className="border-emerald-500/20 bg-[#0a0f12] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <CardContent className="p-8 space-y-8">
          <div className="text-center space-y-1">
            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase leading-none">Consensus Reached</h2>
          ...
          <div className="grid grid-cols-2 border border-white/5 rounded-2xl overflow-hidden divide-x divide-y divide-white/5">
            ...
            <div className="p-4 bg-white/[0.02]">
              <label className="text-[8px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] block mb-1">Network Fee</label>
      ...
      <Card className="border-rose-500/20 bg-[#0f0a0a] overflow-hidden">
        <div className="bg-rose-500/5 px-6 py-10 flex flex-col items-center gap-4 border-b border-rose-500/10 text-center">
           ...
           <div className="space-y-1">
             <h2 className="text-xl font-black text-rose-500 uppercase italic tracking-tighter">Chain Sync Failure</h2>
             <p className="text-xs text-muted-foreground/60 font-bold uppercase tracking-widest">{txResult.error}</p>
           </div>
        </div>
        <CardContent className="p-6">
           <Button className="w-full bg-rose-600 h-12 font-black uppercase" onClick={resetForm}>Retry Submission</Button>
        </CardContent>
      </Card>
    )
  }
    ...
    <Card className={`border-rose-500/20 bg-black/40 backdrop-blur-xl overflow-hidden ${compact ? 'shadow-none border-none' : 'shadow-2xl'}`}>
      ...
      <CardContent className="p-6 space-y-6">
        <div className="space-y-5">
          ...
          {/* Memo */}
          <div className="space-y-2">
             <Label htmlFor="memo" className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Poll Reference ID</Label>
             <Input 
              id="memo"
              value={memo}
